...
def _strategy_block(ctx: GenerationContext) -> str:
    ...
    if _is_entity_like(ctx):
        return (
            ...
            "- For null-field coverage, explicitly call setter(null) for each non-primitive field before assertNull(getter()). Never assert default-null values on a new instance.\n"
...
def _global_rules_block(ctx: GenerationContext) -> str:
    return (
        ...
        "- Null-field tests must set each tested non-primitive field to null via its setter before assertNull(getter()). Skip primitive fields.\n"

...
from junitforge.models import ClassSymbol, Collaborator, FinalizeResult, TemplateSpec
...
def finalize_java(
    ...
    *,
    ...
    # These two rewrites prevent otherwise useful entity/DTO tests from being
    # discarded because the model ignored prompt rules:
    #   1. assertNull(obj.getX()) must explicitly set obj.setX(null) first.
    #   2. Without a proven equality contract, two-instance equality/hashCode
    #      assertions are removed instead of rejecting the whole generated file.
    text = _insert_missing_null_setters_before_assertions(text, cut_symbol)
    text = _remove_unsafe_two_instance_object_assertions(text, cut_symbol)
    ...
    unsafe_object_methods = _unsafe_entity_object_method_assertions(text, cut_symbol, test_package=test_package)
...
def _insert_missing_null_setters_before_assertions(code: str, cut_symbol: ClassSymbol | None) -> str:
    """Insert setX(null) before assertNull(obj.getX()) when compile-safe.

    This is intentionally a small deterministic repair, not a broad rewrite.
    It handles the common entity/DTO failure where the model asserts default
    null values on a freshly constructed object.  The setter is inserted only
    when the CUT is known to expose that setter, including Lombok-generated
    setters already synthesized into ClassSymbol.
    """
    if cut_symbol is None or not cut_symbol.fields:
        return code

    allowed_methods = _allowed_cut_method_names(cut_symbol)
    nullable_fields = [
    ...
    for field in nullable_fields:
        ...
        if setter not in allowed_methods:
            continue

        # Match one-line assertions.  Generated tests in this engine normally
        # emit null-field assertions in this form; multiline assertions remain
        # covered by the rejection gate below.
        pattern = re.compile(
            rf"(?P<indent>^[ \t]*)assertNull\s*\(\s*(?P<var>[A-Za-z_$][\w$]*)\s*\.\s*{re.escape(getter)}\s*\(\s*\)\s*\)\s*;",
            flags=re.MULTILINE,
        )
        ...
        def repl(match: re.Match[str]) -> str:
            ...
            # Inspect the current test-method body before this assertion.  If
            # the setter is already called, preserve the original assertion.
            prefix_start = out.rfind("@Test", 0, match.start())
            body_prefix = out[prefix_start if prefix_start >= 0 else 0:match.start()]
            if re.search(rf"\b{re.escape(var_name)}\s*\.\s*{re.escape(setter)}\s*\(\s*null\s*\)", body_prefix):
                return match.group(0)
            return f"{indent}{var_name}.{setter}(null);\n{match.group(0)}"

        out = pattern.sub(repl, out)
    return out


def _remove_unsafe_two_instance_object_assertions(code: str, cut_symbol: ClassSymbol | None) -> str:
    """Drop fragile two-instance equality assertions when no contract exists.

    Previous v2 behavior rejected the entire generated file when the LLM added
    assertEquals/assertNotEquals between two CUT instances without a detected
    equality contract.  That caused generation_failed for otherwise useful
    entity tests.  This v3 behavior keeps the file and removes only the unsafe
    assertions.  Same-instance, null, different-type, hashCode self, and
    toString smoke assertions remain intact.
    """
    if cut_symbol is None or _has_equality_contract(cut_symbol):
        return code
    ...
    for line in lines:
        ...
        if brace_depth <= 0 and "}" in line:
            ...
            if method_start_index is not None:
                ...
                method_text = _strip_unsafe_two_instance_lines_from_method(method_text, cut_symbol.name)
...
def _strip_unsafe_two_instance_lines_from_method(method_text: str, cut_name: str) -> str:
    vars_for_cut = _cut_vars_in_body(method_text, cut_name)
    if len(vars_for_cut) < 2:
        return method_text
    ...
    for line in method_text.splitlines():
        ...
        if re.search(rf"\bassert(?:Equals|NotEquals)\s*\(\s*({var_alt})\s*,\s*({var_alt})\s*\)\s*;", line):
            m = re.search(rf"\bassert(?:Equals|NotEquals)\s*\(\s*({var_alt})\s*,\s*({var_alt})\s*\)", line)
            if m and m.group(1) != m.group(2):
                continue
        if re.search(rf"\bassertEquals\s*\(\s*({var_alt})\s*\.\s*hashCode\s*\(\s*\)\s*,\s*({var_alt})\s*\.\s*hashCode\s*\(\s*\)\s*\)\s*;", line):
            m = re.search(rf"\bassertEquals\s*\(\s*({var_alt})\s*\.\s*hashCode\s*\(\s*\)\s*,\s*({var_alt})\s*\.\s*hashCode\s*\(\s*\)", line)
            if m and m.group(1) != m.group(2):
                continue
        cleaned.append(line)
    return "\n".join(cleaned)
    ...
    for body in _test_method_bodies(code) or [code]:
        for field in nullable_fields:
            ...
            for m in re.finditer(rf"\bassertNull\s*\(\s*([A-Za-z_$][\w$]*)\s*\.\s*{re.escape(getter)}\s*\(\s*\)\s*\)", body):
                ...
                if not re.search(rf"\b{re.escape(var_name)}\s*\.\s*{re.escape(setter)}\s*\(\s*null\s*\)", before_assert):
                    bad.add(f"{field.name} via {getter}()")
    return sorted(bad)


def _unsafe_entity_object_method_assertions(code: str, cut_symbol: ClassSymbol | None, *, test_package: str | None = None) -> list[str]:
    ...
    bad: set[str] = set()
    allow_two_instance_equality = _has_equality_contract(cut_symbol)
    ...
    for body in _test_method_bodies(code) or [code]:
        ...
        for m in re.finditer(rf"\bassert(?:Equals|NotEquals)\s*\(\s*({var_alt})\s*,\s*({var_alt})\s*\)", body):
            if m.group(1) != m.group(2) and not allow_two_instance_equality:
                bad.add("two-instance equals/assertNotEquals without equality contract")

        for m in re.finditer(rf"\bassertEquals\s*\(\s*({var_alt})\s*\.\s*hashCode\s*\(\s*\)\s*,\s*({var_alt})\s*\.\s*hashCode\s*\(\s*\)\s*\)", body):
            if m.group(1) != m.group(2) and not allow_two_instance_equality:
                bad.add("two-instance hashCode comparison without equality contract")

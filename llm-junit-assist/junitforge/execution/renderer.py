...
def _render_payload_schemas(method: MethodExecutionContext) -> list[str]:
    ...
    for schema in method.payload_schemas:
        ...
        lines.append(
            f"- {schema.type_name} ({schema.resolution_status.value}, depth={schema.depth}{recursive})"
        ...
        if schema.properties:
            ...
            for prop in schema.properties:
                ...
                lines.append(
                    f"    - {prop.name}: {prop.type_name}; readable={str(prop.readable).lower()}, "
                    f"writable={str(prop.writable).lower()}"
...
def render_execution_context(context: ExecutionContext, method_id: str | None = None) -> str:
    ...
    lines.extend([
        ...
        "- Generate all meaningful tests for every listed branch, nested branch, null guard, return, and throw of this one method.",
        ...
        "- Direct @Value fields are already injected by the deterministic skeleton; use exact-field ReflectionTestUtils overrides only for configuration-dependent branches.",
        "- Stub only actual collaborator invocations listed for that branch/helper path.",

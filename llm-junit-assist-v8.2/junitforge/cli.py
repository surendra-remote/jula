...
def run(cfg: AppConfig) -> int:
    ...
    if ant:
        ...
        # The same backend performs the mandatory complete-class JUnit run.
        # JaCoCo measurement remains conditional on --coverage.
        coverage_runner = AntCoverageRunner(
            repo_root=repo,
            cache_dir=repo / ".junitforge",
            toolchain=tc,
            gate=gate,
        )
    else:
        ...
        coverage_runner = CoverageRunner(
            repo_root=repo,
            offline=cfg.gen.offline,
            settings_file=cfg.gen.settings_file,
            local_repo=cfg.gen.local_repo,
            mvn_args=cfg.gen.mvn_args,
        )
    ...
    for i, src in enumerate(targets, 1):
        ...
        elif outcome.status in {"generation_failed", "test_failed", "skipped", "integration_only"}:
    ...
    print(json.dumps({
        ...
        "test_failed": sum(1 for o in outcomes if o.status == "test_failed"),

...
from junitforge.coverage_run import CoverageRunResult, parse_surefire_reports
from junitforge.models import ClassExecutionResult, CompileError, ModuleInfo
...
class AntCoverageRunner:
    ...
    def execute_class(
        ...
        *,
        ...
        """Execute one compiled generated class through JUnit ConsoleLauncher."""
        if not self.available():
            return ClassExecutionResult(False, False, False, note="java/toolchain unavailable")
        java = shutil.which("java") or "java"
        report_dir = self.cache_dir / "junit-reports" / test_file.stem
        ...
        for report in report_dir.glob("*.xml"):
            try:
                ...
                pass
        run_cp = join_classpath([
            str(self.gate.main_classes),
            str(self.gate.test_classes),
            *harvest_jars(self.repo_root),
            *self.toolchain.test_classpath(),
        ])
        cmd = [
            java,
            "-cp",
        ...
        try:
            ...
            return ClassExecutionResult(True, False, False, note=f"junit run failed: {exc}")
        ...
        if proc.returncode != 0 and result.ok:
            result.ok = False
            result.note = "JUnit tests passed but ConsoleLauncher returned a non-zero result"
        return result

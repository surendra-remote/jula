...
def build_context(
    *,
    ...
    equality_descriptor = None
    if template.style in {"dto-pojo", "entity-pojo"}:
        try:
            equality_descriptor = resolve_equality_descriptor(
                symbol,
                source_file.source,
                symbol_lookup if callable(symbol_lookup) else None,
            )
        except Exception as exc:  # noqa: BLE001
            equality_descriptor = EqualityDescriptor(
                strategy=EqualityStrategy.UNRESOLVED,
                assertion_mode=EqualityAssertionMode.CONSERVATIVE,
                entity=template.style == "entity-pojo",
                warnings=[
                    "equality analysis could not be completed safely: "
                    f"{type(exc).__name__}: {str(exc)[:160]}"
                ],
            )
            symbol.equality_descriptor = equality_descriptor
    return GenerationContext(
        ...
        equality_descriptor=equality_descriptor,
